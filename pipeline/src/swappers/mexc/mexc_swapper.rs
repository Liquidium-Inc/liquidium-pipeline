...
fn quote_sell_from_bids(bids: &[OrderBookLevel], amount_in: f64) -> AppResult<f64> {
    if amount_in <= 0.0 {
...
fn quote_buy_from_asks(asks: &[OrderBookLevel], quote_in: f64) -> AppResult<f64> {
    if quote_in <= 0.0 {
        return Err("amount_in must be positive".into());
    }
