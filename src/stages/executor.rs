...
impl<A: PipelineAgent> PipelineStage<ExecutorRequest, SwapResult> for KongSwapExecutor<A> {
    async fn process(&self, executor_request: ExecutorRequest) -> Result<SwapResult, String> {
        let args = Encode!(&self.account_id.owner, &executor_request.liquidation)
        ...
        Ok(SwapResult {
            received_asset: swap_result.receive_symbol,
            received_amount: swap_result.receive_amount,
        })
    }
}
...
mod tests {
    ...
    use crate::{
        executors::kong_swap::{
            ...
            types::{SwapAmountsReply, SwapArgs, SwapReply},
        },
    ...
    use lending::interface::liquidation::{
        LiquidationAmounts, LiquidationRequest, LiquidationResult,
    };
    ...
    async fn test_kong_executor_process_success() {
        ...
        let liquidation_result_response =
            Encode!(&Ok::<_, String>(liquidation_result.clone())).unwrap();
