...
pub trait TransferActions: Send + Sync {
    async fn transfer(&self, token: &ChainToken, to: &ChainAccount, amount_native: Nat) -> Result<String, AppError>; // return tx hash/id

    // Optional ICRC-2-style approval for the current account (used to bump ledger activity).
    async fn approve(&self, token: &ChainToken, spender: &ChainAccount, amount_native: Nat)
